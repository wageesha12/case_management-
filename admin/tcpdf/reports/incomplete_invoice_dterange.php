...
if(isset($_POST['btn_summeryinvoicecomplete'])){
	...
	if($stdte==null || $enddte==null){
		header("location:../../incomplete_invoicemenu.php");
	}
	else if($stdte>$enddte){
		echo "<script>
			alert('Invalid Date Selection');
			window.location.href='../../incomplete_invoicemenu.php';
		</script>";
	}
	else{
		...
		// Logo
			...
			$html ='<table border="0">
						<tr>
							...
							$html.= '<tr>
								<td width="30%"></td>
								<td width="40%" style="color:#001f4d"><h3 align="center">Incomplete Invoice </h3> </td>
							   <td width="30%" style="color:#001f4d"><h5 align="right"> Incomplete Invoice Report </h5></td>
							 </tr>';
							$html.='<tr>
						  <td width="30%"></td>
...
$pdf->SetTitle('Incomplete Invoices');
...
$pdf->AddPage();
	...
	$sql1="SELECT * FROM service_master INNER JOIN vehical_cus_master 
				...
				WHERE service_master.date>='$stdte' AND 
					...
					service_master.finish_status=0 AND
...
$pdf->Output('Incomplete_Invoice_'.$stdte.'_'.$enddte.'.pdf', 'I');
