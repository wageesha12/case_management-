						...
						$html ='<table border="0"  width="100%">
									<tr>
										<td width="100%" >
											...
											<div style="font-size:18px;font-weight:bold;" align="center">Complete Discusss Case</div>
											<div style="font-size:13px;" align="center">'.$_GET['st'].' to '.$_GET['ed'].'</div>
...
$pdf->SetTitle('Complete Discusss');
			...
			<table border="1" width="100%">
				<thead>
					<tr>
						...
						<th> Case Step</th>
						<th> Prepair Points</th>
						<th> Observation</th>
					</tr>
				</thead>
				<tbody>';
				$sql13="SELECT * FROM casecycle_details INNER JOIN case_details ON casecycle_details.casedetails_key=case_details.casedetail_key
																	WHERE case_details.lawyer_key='$ses_ukey'
																	AND casecycle_details.nextcourt_dte>='$_GET[st]'
																	AND casecycle_details.nextcourt_dte>='$_GET[ed]'
																	AND case_details.finact=0
																	AND casecycle_details.finact=0";
				...
				while($row13=mysqli_fetch_array($result13)){
					...
					$html.='<tr style="font-size:10px;">';
						$html.='<td> '.$row13['nextcourt_dte'].'</td>';
						$html.='<td> '.$row13['case_code'].'</td>';
						$html.='<td> '.$b1.'</td>';
						$html.='<td> '.$c1.'_'.$d1.'</td>';
						$html.='<td> '.$e1.'</td>';
						$html.='<td> '.$f1.'</td>';
						$html.='<td> '.$row13['status_cycle'].'</td>';
						$html.='<td>';
							$sql15="SELECT * FROM prepairpoints_details WHERE casecycle_key='$row13[casecycle_key]' AND finact=0";
							$result15 = mysqli_query($link,$sql15);
							while($row15=mysqli_fetch_array($result15)){
								$html.=' '.$row15['points'].'<br>';
							}
						$html.='</td>';
						$html.='<td>';
							$sql14="SELECT * FROM observation_details WHERE casecycle_key='$row13[casecycle_key]' AND finact=0";
							$result14 = mysqli_query($link,$sql14);
							while($row14=mysqli_fetch_array($result14)){
								$html.=' '.$row14['observationn'].'<br>';
							}
						$html.='</td>';
					$html.='</tr>';
				}
...
$pdf->Output('Completediscuss'.$_GET['st'].'_'.$_GET['ed'].'.pdf', 'I');
}
//============================================================+
// END OF FILE
//============================================================+
